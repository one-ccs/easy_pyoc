...
from typing import Union, Any, Sequence, overload, TypeVar, Tuple, Literal
...
@overload
def quick_data(key: str, flat: Literal[True] = ...) -> str: ...
@overload
def quick_data(key: str, flat: Literal[False]) -> list[str]: ...

# 单键（带类型元组）重载：用户提供类型，返回值为该类型
@overload
def quick_data(key: Tuple[str, T], flat: Literal[True] = ...) -> T: ...
@overload
def quick_data(key: Tuple[str, T], flat: Literal[False]) -> T: ...

# 单键（带类型与默认值的元组）
@overload
def quick_data(key: Tuple[str, T, Any], flat: Literal[True] = ...) -> T: ...
@overload
def quick_data(key: Tuple[str, T, Any], flat: Literal[False]) -> T: ...

# 可变键：返回结果元组
@overload
def quick_data(*keys: Sequence[object], flat: Literal[True] = ...) -> tuple: ...
@overload
def quick_data(*keys: Sequence[object], flat: Literal[False]) -> tuple: ...

def quick_data(*keys, flat: bool = True) -> Union[tuple, dict, Any]:
    """从当前请求快速提取值并按请求键返回。
